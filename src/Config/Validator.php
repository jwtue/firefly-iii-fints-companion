...
final class Validator
{
    ...
    public function validateLogin(array $data, bool $isNew): array
    {
        ...
        if ($isNew && trim((string) ($data['bank_password'] ?? '')) === '') {
            $errors['bank_password'] = 'This field is required.';
        }
    ...
    public function validateAccount(array $data): array
    {
        ...
        if (!preg_match('/^[A-Za-z0-9._-]+$/', $slug)) {
            $errors['slug'] = 'Only letters, digits, dot, dash and underscore; no spaces.';
        }

        if ((int) ($data['login_id'] ?? 0) <= 0) {
            $errors['login_id'] = 'Please choose a login.';
        }

        $selector = ($data['selector_type'] ?? 'iban') === 'number' ? 'number' : 'iban';
        if ($selector === 'iban' && trim((string) ($data['bank_account_iban'] ?? '')) === '') {
            $errors['bank_account_iban'] = 'Required when selecting by IBAN.';
        }
        if ($selector === 'number' && trim((string) ($data['bank_account_number'] ?? '')) === '') {
            $errors['bank_account_number'] = 'Required when selecting a credit card by account number.';
        }

        if (trim((string) ($data['firefly_account_id'] ?? '')) === '') {
            $errors['firefly_account_id'] = 'This field is required.';
        }
        ...
        if ($cron !== '' && !CronExpression::isValidExpression($cron)) {
            $errors['schedule_cron'] = 'Not a valid cron expression.';
        }
    ...
    {
        if ($from === '' || $to === '') {
            return 'Both start and end of the window are required.';
        }
        // The importer passes these straight into DateTime, so resolve them the same way.
        $fromTs = strtotime($from);
        $toTs = strtotime($to);
        if ($fromTs === false || $toTs === false) {
            return 'Could not understand the date window (e.g. "now - 7 days" .. "now").';
        }
        if ($fromTs > $toTs) {
            return 'The start of the window must not be after its end.';
        }
        if (($toTs - $fromTs) > self::MAX_WINDOW_DAYS * 86400) {
            return 'The window must not exceed ' . self::MAX_WINDOW_DAYS . ' days (PSD2 forces a second TAN beyond that).';
        }
